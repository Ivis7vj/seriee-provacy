Privacy Policy for SERIEE

Last updated: [DATE]

SERIEE is a series discovery, tracking, and review application. SERIEE is not a streaming platform.

Information We Collect:
- Account information (username, email) if you create an account
- App usage data such as watched, liked, rated, or watchlisted series
- Push notification token (used only to send relevant notifications)

What We Do NOT Collect:
- No Aadhaar number
- No payment or financial information
- No phone contacts
- No background audio/video
- No real-time location tracking

How We Use Information:
- To personalize your experience
- To send optional notifications about series updates
- To improve app performance and features

Third-Party Services:
SERIEE may use third-party services such as:
- Firebase (authentication, notifications)
- TMDB (series metadata)
- News sources (links only, no content ownership)

These services may collect information according to their own privacy policies.

Data Storage:
- User data is securely stored
- Cached data may be cleared periodically
- Users can request deletion of their data

Children’s Privacy:
SERIEE is not intended for children under 13.

Changes to This Policy:
We may update this policy from time to time.

Contact Us:
If you have questions about this Privacy Policy, contact us at:
support@seriee.app
